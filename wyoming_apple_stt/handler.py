...
class AppleSTTEventHandler(AsyncEventHandler):
    ...
    Handles the Wyoming protocol event flow: Describe, Transcribe, AudioChunk,
    AudioStop for STT (streamed to a pre-warmed apple-stt worker, with a
    buffered one-shot fallback) and Synthesize plus the synthesize-start/chunk/
    stop streaming events for TTS (delegated to a pre-warmed apple-tts worker).
    """
    ...
    async def disconnect(self) -> None:
        ...
        A worker abandoned mid-request has unread frames in its pipe and cannot
        be reused, so it is stopped rather than released.
        """
